...
import com.stock.oauth2.authserver.document.MongoRefreshToken;
...
public class MongoTokenStore implements TokenStore {
	...
	public OAuth2Authentication readAuthentication(String token) {
		...
		query.addCriteria(Criteria.where(AuthConstant.TOKEN_ID).is(extractTokenKey(token)));
	...
	public OAuth2AccessToken readAccessToken(String tokenValue) {
		Query query = new Query();
		query.addCriteria(Criteria.where(AuthConstant.TOKEN_ID).is(extractTokenKey(tokenValue)));
	...
	public void removeAccessToken(OAuth2AccessToken oAuth2AccessToken) {
		Query query = new Query();
		query.addCriteria(Criteria.where(AuthConstant.TOKEN_ID).is(extractTokenKey(oAuth2AccessToken.getValue())));
	...
	public OAuth2Authentication readAuthenticationForRefreshToken(OAuth2RefreshToken refreshToken) {
		Query query = new Query();
		query.addCriteria(Criteria.where(AuthConstant.TOKEN_ID).is(extractTokenKey(refreshToken.getValue())));
	...
	public void removeAccessTokenUsingRefreshToken(OAuth2RefreshToken refreshToken) {
		Query query = new Query();
		query.addCriteria(Criteria.where(AuthConstant.REFRESH_TOKEN).is(extractTokenKey(refreshToken.getValue())));
		mongoTemplate.remove(query, MongoAccessToken.class);
	}
	...
	public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
		...
		Query query = new Query();
		query.addCriteria(Criteria.where(AuthConstant.AUTHENTICATION_ID).is(authenticationId));
	...
	public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String username) {
		...
		query.addCriteria(Criteria.where(AuthConstant.USERNAME).is(username));
